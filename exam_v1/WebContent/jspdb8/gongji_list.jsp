...
<meta charset="UTF-8">
<script
	src="http://netdna.bootstrapcdn.com/bootstrap/3.3.5/js/bootstrap.js"></script><!-- 부트스트랩 -->
<link
...
<body>
	<table cellspacing=1 width=600 border=1>
		...
		int total = 0;
		if (search != null) {
			for (Notice2 n : service.selectAll(what, search)) {
				total++;
			}
		} else {
		...
		if (search != null) {
			for (Notice2 n : service.selectAll(what, search)) { // 모든 글 검색 및 뿌리기
				if (LineCnt < start) {
			LineCnt++;
			continue;
				}
		...
		for (Notice2 n : service.selectAll()) { // 모든 글 검색 및 뿌리기
	...
	<table width=650>
		<tr>
			<td width=546>
				...
				Onclick="window.location='gongji_list.jsp'">목록</button> <%
 	}
 %>
			</td>
	...
	<div class="center">
		...
		<nav aria-label="Page navigation">
			<ul class="pagination">
				...
				%>
				<li><a href="./gongji_list.jsp?num=1&cnt=<%=cntPT%>"
					aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
				<%
					} else {
				%>
				<li><a
					href="./gongji_list.jsp?num=<%=startPage - pageSize%>&cnt=<%=cntPT%>"
					aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
				<%
					}
				for (int i = startPage; i < endPage; i++) { // 페이지번호 출력
				%>
				<li><a href="./gongji_list.jsp?num=<%=i%>&cnt=<%=cntPT%>"><%=i%></a></li>
				<%
					}
				if (startPage + pageSize > pageCount) {
				%>
				<li><a
					href="./gongji_list.jsp?num=<%=pageCount%>&cnt=<%=cntPT%>">&raquo;</a>
				<li>
					<%
						} else {
					%>
